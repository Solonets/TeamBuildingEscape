...
public class RandomFiller {
    ...
    static InventoryItem getRandomInventory()
    {
        ...
        String[] pickedMessages = {
                ...
                "Очки всегда добавляют важности в разговоре, поправляя их пальцем после каждой фразы, ты добавляешь весомости сказанному. Теперь ты также можешь различать буквы на экране",
        ...
        int[] codingSkills = {0, 1, 0, 0, 2, 5, 0, 0, 0, 0};
        int i = 0;
        while(random.nextInt(3) <= 1 && i < items.length - 1) {i++;}
        return new InventoryItem(items[i], accusative[i], pickedMessages[i], influences[i], codingSkills[i], locations[random.nextInt(locations.length)]);
    }
